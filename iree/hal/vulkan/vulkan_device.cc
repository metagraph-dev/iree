...
static void iree_hal_vulkan_device_initialize_command_queues(
  ...
  iree::hal::vulkan::DynamicSymbols* syms =
      device->logical_device->syms().get();
...
static iree_status_t iree_hal_vulkan_device_wait_idle_with_deadline(
  ...
  if (deadline_ns == IREE_TIME_INFINITE_FUTURE) {
    ...
    return VkResultToStatus(device->logical_device->syms()->vkDeviceWaitIdle(
...
static const iree_hal_device_vtable_t iree_hal_vulkan_device_vtable = {
